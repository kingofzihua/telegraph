...
function initQuill() {
            ...
            if (context.format.blockHeader || context.format.blockSubheader || context.format.blockBlockquote || context.format.blockPullquote) {
                if (isEnd) {
                    this.quill.formatLine(index, 1, {
                        blockHeader: false, blockSubheader: false, blockBlockquote: false, blockPullquote: false
                    }, Quill.sources.USER);
                }
            }
        } else {
    ...
    var quill = new MyQuill("#_tl_editor", {
        ...
        modules: {
            clipboard: {
                matchers: [getSimpleMatcher("h2", {blockHeader: true}), getSimpleMatcher("h5", {blockSubheader: true}), getSimpleMatcher("h6", {blockSubheader: true}), ['img', function (node, delta) {
                ...
                bindings: {
                    indent: {
                        ...
                        key: Keyboard.keys.TAB, shiftKey: true, handler: function (range, context) {
                            let block = null;
                            if (range.length > 0) {
                        ...
                        key: Keyboard.keys.TAB, shiftKey: false, handler: function (range, context) {
                            let block = null;
                            if (range.length > 0) {
                                let blocks = quill.scroll.descendants(Block, range.index, range.length);
                                if (blocks.length != 1) {
                        ...
                        key: Keyboard.keys.TAB, shiftKey: null, handler: function (range, context) {
                            return false;
                        }
                    }, "detect embed": {
                        key: Keyboard.keys.ENTER, collapsed: true, handler: function (range, context) {
                            let [line_blot, offset] = quill.scroll.line(range.index);
                            if (line_blot) {
                                let text = line_blot.domNode.innerText;
                                let prefix = text.substr(0, offset);
                                ...
                                if (match = prefix.match(/(^|\s)(https?:\/\/\S+)$/)) {
                                    ...
                                    if (!prefix.substr(0, offset - url.length).trim().length && line_blot.domNode.tagName == 'P') {
                                        ...
                                        if (figure_value) {
                                            ...
                                            quill.updateContents(new Delta().retain(offset).delete(prefix.length).insert({blockFigure: figure_value}), Quill.sources.USER);
                                            hideBlocksTooltip();
                                            return false;
                                        }
                                    }
                        ...
                        key: Keyboard.keys.ENTER, shiftKey: true, handler: breakHandler.bind(quill, true)
                    }, enter: {
                        key: Keyboard.keys.ENTER, handler: breakHandler.bind(quill, false)
                    }, "detect link": {
                        key: " ", collapsed: true, handler: detectLinkHandler
                    }, "cancel placeholder": {
                        key: Keyboard.keys.ESCAPE, handler: function (range, context) {
                        ...
                        key: Keyboard.keys.BACKSPACE, collapsed: true, offset: 0, handler: function (range, context) {
                            let [line_blot, index] = quill.scroll.line(range.index);
                        ...
                        key: Keyboard.keys.BACKSPACE, collapsed: true, offset: 0, handler: function (range, context) {
                            let [line_blot, index] = quill.scroll.line(range.index);
                            if (line_blot && line_blot.prev && line_blot.prev instanceof FieldBlot && $(line_blot.domNode).text().length > 0) {
                                return false;
                            }
                            return true;
                        }
    ...
    quill.on(Quill.events.EDITOR_CHANGE, function (eventType, range) {
        ...
        if (range.length === 0) {
            hideFormatTooltip();
            if (block != null && !(block instanceof FieldBlot) && !(block instanceof BlockquoteBlot) && !(block instanceof PullquoteBlot) && !(block instanceof CodeBlock) && !(block instanceof ListItem) && !$(block.domNode).text().length) {
    ...
    quill.on(Quill.events.SCROLL_OPTIMIZE, function (mutations) {
        mutations.forEach((mutation) => {
            if (mutation.type == 'childList' && !mutation.addedNodes.length && mutation.removedNodes.length) {
                ...
                } else if (next && prev && !(prev.tagName == 'BR' && prev.className == 'inline') && (next.tagName == 'BR' && next.className == 'inline') && !next.nextSibling) {
                    if (next.parentNode) {
...
function checkRequiredBlots(quill) {
    ...
    if (first instanceof BlockEmbed) {
        quill.updateContents(new Delta()
            .insert('\n', {blockTitle: true})
            .insert('\n', {blockAuthor: true}), Quill.sources.SILENT);
    } else {
        ...
        return loadImage(file, (canvas) => {
            if (canvas.type === 'error') {
                ...
                if (canvas.toBlob) {
                    ...
                    let file_data = {
                        type: file.type, base64_data: dataurl.split(',')[1]
                    };
...
// compute_width
function _uploadFile(file_data, compute_width, succ_func, err_func) {
    var r = new FormData;
    r.append("file", uploadDataToBlob(file_data));
    $.ajax({
        url: "/upload",
        type: "POST",
        data: r,
        ...
        xhr: function () {
            ...
            req.upload.addEventListener("progress", function (evt) {
                evt.lengthComputable && compute_width && compute_width(evt.loaded, evt.total)
            });
            return req
        },
        beforeSend: function (t) {
            compute_width && compute_width(0, 1)
        },
        success: function (t) {
            if (t.error) {
                return err_func && err_func(t.error)
            }

            $.each(t, function (t, data) {
                succ_func && succ_func(data)
            })
        },
        error: function (t) {
            return err_func && err_func("Network error")
        }
    })
}
...
function getPageContent(for_draft) {
    ...
    if (!for_draft) {
        ...
        return {
            data: JSON.stringify(wrapDomElement($node.get(0)).children), length: $node.html().length
        };
    } else {
...
function savePage() {
    ...
    let meta = $('img[src^="data:"],video[src^="data:"]');
    ...
    if (content.length > 65536) return showError("Content is too big");

    $tl_article.addClass("tl_article_saving");

    updateEditable(false);

    let s = "---------------------------TelegraPhBoundary21";
...
function checkAuth() {
    ...
    $.ajax(T.apiUrl + "/check", {
        ...
        success: function (result) {
            if (result.save_hash) {
                T.saveHash = result.save_hash
            }

            if (result.report_hash) {
                T.reportHash = result.report_hash;
                $tl_page_footer.addClass("tl_page_footer_visible")
            }

            if (result.can_edit && T.saveHash || !T.pageId) {
                result.short_name && $account.text(result.short_name);
            ...
            if (!draftGet() && result.author_name) {
                let author = result.author_url ? {link: result.author_url} : {};
                let [blot] = quill.scroll.descendants(AuthorBlot);
                if (blot) {
                    quill.updateContents((new Delta).retain(blot.offset()).delete(blot.length()).insert(result.author_name, author), Quill.sources.USER)
                }
            }

            if (result.auth_alert && result.short_name) {
                let msg = '<i class="tl_alert_success"></i><h4>Success!</h4><p>You are now logged in as <b>' + htsc(result.short_name) + "</b> in this browser.</p>";

                if (result.migrate_count > 0 && result.migrate_hash) {
                    msg += "<p>We can also add " + result.migrate_count + " Telegraph page" + (result.migrate_count > 1 ? "s" : "") + " from this browser to your account.</p>";
                    showAlert(msg, {
                        close_btn: "Skip", submit_btn: "Add", submit: function () {
                            migratePages(result.migrate_hash)
                        }
                    })
                } else {
                    showAlert(msg, {close_btn: "Continue"})
                }
            }
...
function migratePages(migrate_hash) {
    $.ajax(T.apiUrl + "/migrate", {
        data: {migrate_hash: migrate_hash},
        type: "POST",
        dataType: "json",
        xhrFields: {withCredentials: true},
        success: function (data) {
            if (data.migrated_count > 0) {
                showAlert('<i class="tl_alert_success"></i><h4>Success!</h4><p>Added <b>' + data.migrated_count + "</b> Telegraph page" + (data.migrated_count > 1 ? "s" : "") + ' to your account.</p><p>To see a list of your pages, talk to the <a href="https://t.me/telegraph" target="_blank">@Telegraph</a> bot on Telegram.</p>', {close_btn: "Continue"})
            } else {
                hideAlert()
            }
        }
...
function updateEditableText(context, is_edit) {
    ...
    if (is_edit) {
        ...
        $('.editable_text > .editable_input', context).map(function () {
...
function updateReportFormState() {
    let report = $report_form.get(0);
    let reason = !report.reason.value || "copyright" == report.reason.value;

    $report_form.toggleClass("copyright_selected", "copyright" == report.reason.value);
    $report_form.toggleClass("form_disabled", !!reason);
    report.reason.value && report.comment.focus();
}

function sendReport() {
    if ($report_form.hasClass("form_disabled")) return false;
    let report = $report_form.get(0);
    $report_popup.addClass("tl_popup_loading");
    $.ajax(T.apiUrl + "/report", {
        data: {
            page_id: T.pageId, report_hash: T.reportHash, reason: report.reason.value, reason_text: report.comment.value
        }, type: "POST", dataType: "json", xhrFields: {withCredentials: true}, success: function (t) {
            $report_popup.removeClass("tl_popup_loading");
            hidePopup();
            showAlert('<i class="tl_alert_success"></i><h4>Report sent</h4><p>Telegraph moderators will review your report. Thank you!</p>', {close_btn: "Done"})
        }
    })
}
...
function showLinkTooltip(link, value) {
    if (!isEdit()) return;
    let range = {
        index: link.offset(quill.scroll), length: link.length()
    };
...
function showAlert(html, options) {
    ...
    hideAlert();
    let $alert = $('<div class="tl_popup tl_alert"><main class="tl_popup_body tl_alert_message"><section></section><aside class="tl_popup_buttons"></aside></main></div>');
    $("section", $alert).html(html);
    let $aside = $("aside", $alert);
    if (options.close_btn) {
        let $close_btn = $('<button class="button"></button>');
        $close_btn.html(options.close_btn).click(options.close).appendTo($aside)
    }
    if (options.submit_btn) {
        let $submit_btn = $('<button class="button"></button>');
...
function isOverElement(bounds1, $elem2, padding) {
    ...
    if ((bounds1.left - bounds2.right >= padding || bounds2.left - bounds1.right >= padding) || (bounds1.top - bounds2.bottom >= padding || bounds2.top - bounds1.bottom >= padding)) {
        return false;
    }
    return bounds2;
}
...
function tooltipUpdatePosition($tooltip, range, options) {
    ...
    let tt = {
        width: $tooltip.outerWidth(), height: $tooltip.outerHeight(),
    }
    let win = {
        width: $(window).outerWidth(), height: $(window).outerHeight(), scrolltop: document.body.scrollTop
    }
    let min = {
        left: 9, top: win.scrolltop + 9,
    };
    let max = {
        left: win.width - tt.width - 9, top: win.scrolltop + win.height - tt.height - 9,
    };
    ...
    if (options.minDelta && Math.abs(tt.left - $tooltip._left) < options.minDelta && Math.abs(tt.top - $tooltip._top) < options.minDelta) {
        return;
    }
...
class BlockAuto extends Block {
    ...
    optimize() {
        super.optimize()
        let direction = window.getComputedStyle(this.domNode).direction;
        if (direction == "rtl") {
...
class FigureBlot extends BlockEmbed {
    ...
    constructor(domNode, value) {
        ...
        $(this.domCaption).on("selectionchange", (e) => {
            let selection = window.getSelection();
            if (selection && selection.focusNode === this.domCaption && selection.isCollapsed) {
                this.focusCaptionInput(e)
            }
        })
    ...
    focusCaptionInput(e) {
        if (this.domCaption.classList.contains("empty")) {
            let input = this.domCaption.querySelector(".editable_input");
            if (input) {
                e.preventDefault();
                input.focus()
            }
    ...
    appendVideoNode(src) {
        ...
        video.addEventListener('loadeddata', function () {
            if (!this.mozHasAudio && !(this.webkitAudioDecodedByteCount) && !(this.audioTracks && this.audioTracks.length)) {
    ...
    uploadFile(file_data) {
        _uploadFile(file_data, (loaded, total) => {
            let persent = 0;
            if (total && loaded) {
...
let $tl_page = $(".tl_page");
let $tl_page_footer = $(".tl_page_footer");
...
let $report_button = $("#_report_button");
let $report_popup = $("#_report_popup");
let $report_form = $("#_report_form");
let $report_cancel = $("#_report_cancel");
let formatTTOptions = {padding: 10, position: browser.mobile ? "bottom" : "top", minDelta: 5};
let linkTTOptions = {padding: 7, position: "bottom", depend: $tl_tooltip, dependPadding: 10};
...
$link_button.click(function (e) {
    ...
    if (active) {
        ...
        toolbarPrompt($tl_tooltip, 'Paste or type a link...', function (value) {
            value = value.trim();
            if (value.substr(0, 1) != '#' && value.substr(0, 1) != '/' && value.substr(0, 7) != 'http://' && value.substr(0, 8) != 'https://' && value.substr(0, 7) != 'mailto:') {
                if (value.indexOf('@') > 0) {
                    value = 'mailto:' + value;
                } else {
                    value = 'http://' + value;
...
$image_button.click(function () {
    ...
    if (fileInput == null) {
        ...
        fileInput.addEventListener("change", function () {
            if (null != fileInput.files && null != fileInput.files[0]) {
                var file = fileInput.files[0];
                updatePhoto(file, function (f) {
                    if (quill.fileSizeLimit && f.size > quill.fileSizeLimit) {
                    ...
                    reader.onload = function (e) {
                        ...
                        if (figure_value) {
                            let range = quill.getSelection(true);
                            quill.updateContents((new Delta).retain(range.index)["delete"](range.length)
                                .insert({blockFigure: figure_value}), Quill.sources.USER)
                        } else {
...
var quill = initQuill(), pageContent = false;
